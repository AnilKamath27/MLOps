...
@step
def deployment_trigger(accuracy:float, config:DeployementTriggerConfig):
...
def predictor(service: MLFlowDeploymentService,data: str,) -> np.ndarray:
    ...
    columns_for_df = ["Year","Selling_Price","Present_Price","price","Driven_kms","Fuel_Type","Selling_type","Transmission","Owner"]
...
def continuous_deployment_pipeline( data_path:str, min_accuracy:float=0.4, workers:int=1, timeout:int=DEFAULT_SERVICE_START_STOP_TIMEOUT):
        df = ingest_data(data_path)
        X_train, X_test,y_train,y_test= clean_data(df)
        model = train_model(X_train, X_test,y_train,y_test)
        R2Score_value, MeanAbsoluteError_value, MeanSquaredError_value = evaluate_model(model, X_test,y_test)
        deployment_decision = deployment_trigger(R2Score_value)
...
def inference_pipeline(pipeline_name: str, pipeline_step_name: str):
    ...
    service = prediction_service_loader(pipeline_name=pipeline_name,pipeline_step_name=pipeline_step_name,running=False)
